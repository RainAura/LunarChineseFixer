...
工具会在 Lunar 的 `FontRenderer.onResourceManagerReload` 流程中补充 `clearCaches()` 调用，并修补缓存文字渲染段的纹理绑定。每个渲染段绘制前会先使旧的 OpenGL 纹理状态失效，再绑定该段实际使用的 Unicode 字体页，避免“岛”显示成“涛”这类低字节相同、页码错误的错字。工具不会删除材质包，也不会修改 OptiFine 字体设置。
